...
import { clampCrop, computeMmPerPixel, normalizeDeg, rotatedBoundsMm } from "../core/transform";
...
function commitProject(state: AppState, project: Project): AppState {
  return { ...state, project: { ...project, updatedAt: new Date().toISOString() }, past: pushPast(state.past, state.project), future: [], transientBaseProject: null, saveState: "dirty" };
}

function previewProject(state: AppState, project: Project): AppState {
  return { ...state, project: { ...project, updatedAt: new Date().toISOString() }, transientBaseProject: state.transientBaseProject ?? state.project, saveState: "dirty" };
}

function commitTransientEdit(state: AppState): AppState {
  if (!state.transientBaseProject) return state;
  return { ...state, past: pushPast(state.past, state.transientBaseProject), future: [], transientBaseProject: null, saveState: "dirty" };
}

function deleteObjects(state: AppState, ids: readonly string[]): AppState {
  const idSet = new Set(ids);
  const next = commitProject(state, { ...state.project, objects: state.project.objects.filter((object) => !idSet.has(object.id)) });
...
function duplicateObjects(state: AppState, ids: readonly string[]): AppState {
  ...
  const copies = selected.map((source, index) => ({ ...source, id: generateId("obj"), xMm: source.xMm + 500, yMm: source.yMm + 500, locked: false, zIndex: state.project.objects.length + index }));
...
function undoState(state: AppState): AppState {
  ...
  const next: AppState = { ...committed, project: previous, past: committed.past.slice(0, -1), future: [committed.project, ...committed.future].slice(0, MAX_HISTORY_ENTRIES), transientBaseProject: null, saveState: "dirty" };
...
function redoState(state: AppState): AppState {
  ...
  const next: AppState = { ...committed, project: nextProject, past: [...committed.past, committed.project].slice(-MAX_HISTORY_ENTRIES), future: committed.future.slice(1), transientBaseProject: null, saveState: "dirty" };
  return setSelection(next, next.selectedIds);
}
...
export function appReducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "SET_BACKGROUND": return commitProject(state, {
      ...state.project,
      background: { ...state.project.background, imageDataUrl: action.imageDataUrl, naturalWidthPx: action.naturalWidthPx, naturalHeightPx: action.naturalHeightPx, sourceType: action.sourceType, sourcePage: action.sourcePage, crop: null, rotationDeg: 0 },
    ...
    case "ADD_OBJECT": return setSelection(commitProject(state, { ...state.project, objects: [...state.project.objects, action.object] }), [action.object.id]);
    case "UPDATE_OBJECT": return commitProject(state, { ...state.project, objects: state.project.objects.map((object) => object.id === action.id ? { ...object, ...action.patch, widthMm: Math.max(1, action.patch.widthMm ?? object.widthMm), depthMm: Math.max(1, action.patch.depthMm ?? object.depthMm), heightMm: Math.max(0, action.patch.heightMm ?? object.heightMm), rotationDeg: normalizeDeg(action.patch.rotationDeg ?? object.rotationDeg) } : object) });
    case "MOVE_OBJECT": return commitProject(state, { ...state.project, objects: state.project.objects.map((object) => object.id === action.id && !object.locked ? { ...object, xMm: action.xMm, yMm: action.yMm } : object) });
    case "MOVE_OBJECTS": {
      const moves = new Map(action.moves.map((move) => [move.id, move]));
      const project = { ...state.project, objects: state.project.objects.map((object) => { const move = moves.get(object.id); return move && !object.locked ? { ...object, xMm: move.xMm, yMm: move.yMm } : object; }) };
      return action.preview ? previewProject(state, project) : commitProject(state, project);
    }
    case "ROTATE_OBJECT": {
      const project = { ...state.project, objects: state.project.objects.map((object) => object.id === action.id && !object.locked ? { ...object, rotationDeg: normalizeDeg(action.rotationDeg) } : object) };
      return action.preview ? previewProject(state, project) : commitProject(state, project);
    }
    ...
    case "ALIGN_SELECTED": return commitProject(state, { ...state.project, objects: alignObjects(state.project.objects, state.selectedIds, action.alignment) });
    case "DISTRIBUTE_SELECTED": return commitProject(state, { ...state.project, objects: distributeObjects(state.project.objects, state.selectedIds, action.axis) });
    case "GROUP_SELECTED": {
    ...
    case "UNGROUP_SELECTED": return commitProject(state, { ...state.project, objects: state.project.objects.map((object) => state.selectedIds.includes(object.id) ? { ...object, groupId: null } : object) });
    ...
    case "SELECT_RECT": {
      const ids = state.project.objects.filter((object) => { if (!object.visible) return false; const bounds = rotatedBoundsMm(object); return bounds.minXMm >= action.bounds.minXMm && bounds.maxXMm <= action.bounds.maxXMm && bounds.minYMm >= action.bounds.minYMm && bounds.maxYMm <= action.bounds.maxYMm; }).map((object) => object.id);
