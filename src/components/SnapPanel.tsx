...
export function SnapPanel({ state, dispatch }: Props) {
  ...
  return (
    <section className="snap-panel">
      <h2>スナップ</h2>
      <label className="row"><input type="checkbox" checked={settings.grid} onChange={(e) => patch({ grid: e.target.checked })} />グリッド</label>
      <label className="row"><input type="checkbox" checked={settings.objects} onChange={(e) => patch({ objects: e.target.checked })} />他オブジェクト</label>
