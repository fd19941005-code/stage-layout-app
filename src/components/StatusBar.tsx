  ...
  return (
    <footer className="status-bar">
      <span className={calibration.mmPerPixel === null ? "warning" : ""}>
        {calibration.mmPerPixel === null
          ? "未校正"
          : `校正済み: 1px = ${calibration.mmPerPixel.toFixed(2)}mm`}
      </span>
      <span>
        {cursorMm
          ? `X: ${Math.round(cursorMm.xMm)}mm / Y: ${Math.round(cursorMm.yMm)}mm`
          : "—"}
      </span>
      <span>
        {mA && mB ? `測定: ${Math.round(mmDistance(mA, mB))}mm` : ""}
      </span>
      <span>選択: {state.selectedId ? 1 : 0} / {state.project.objects.length}個</span>
